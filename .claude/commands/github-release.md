...
Read root `package.json` to get current version (e.g., "0.1.0").
...
**Store the selected version type** (patch/minor/major/skip) for use in Step 14.
...
## Phase 2: Release Automation (Steps 10-17)
...
This project has the following check scripts:

| Script | Command | Description |
|--------|---------|-------------|
| `typecheck` | `pnpm exec tsc --noEmit` | TypeScript type checking |
| `lint` | `pnpm lint` | ESLint via Next.js |
| `test` | `pnpm test` | Tests (if script exists in package.json) |
| `build` | `pnpm build` | Prisma generate + Next.js build |

Inform the user: "Will run: typecheck, lint, test (if configured), build"
...
pnpm lint
```
If fails: Stop, show error, output "Fix the lint errors (try: pnpm lint -- --fix) and re-run /github-release"

**Test (conditional):**
First check if a `test` script exists:
```bash
node -p "Boolean(require('./package.json').scripts?.test)"
```
If `true`, run tests:
```bash
pnpm test
...
If `false`, skip with message: "No test script configured, skipping tests."
...
| Lint fails | Stop, show errors, suggest `pnpm lint -- --fix` |
...
1. **Phase 1 - Issue Creation:**
   - Title: "Add recipe clone feature"
   - Labels: enhancement
   - Version: minor (v0.2.0)
   - Creates `.plans/add-recipe-clone-feature.md`
   - Updates CHANGELOG.md
   - Creates GitHub issue #10
   - Generates commit message: `feat(recipes): add recipe clone feature (#10)`

2. **Phase 2 - Release Automation:**
   - Remote check: Up to date
   - Runs typecheck (`tsc --noEmit`)... passed
   - Runs lint (`next lint`)... passed
   - Runs test... skipped (no test script configured)
   - Runs build (`prisma generate && next build`)... passed
...
3. **Final Output:**
   ...
   Issue: https://github.com/oharu121/cook-share/issues/10
   Version: 0.1.0 -> 0.2.0
   Tag: v0.2.0
   Release: https://github.com/oharu121/cook-share/releases/tag/v0.2.0
   ```
